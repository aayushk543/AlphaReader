"""Full-image OCR scanning — segment characters and recognise text."""
...
def segment_characters(image_path: str):
    """Segment individual characters from a full image using contours.

    Returns a list of PIL Images sorted left-to-right, top-to-bottom,
    along with their bounding boxes.
    """
    try:
        import cv2
    except ImportError:
        print("❌ OpenCV is required for scanning. Install it:\n"
    ...
    if not boxes:
        print("⚠️  No characters detected in the image.")
        return [], []
...
def scan_image(image_path: str):
    """Segment characters from a full image and recognise each one."""
        ...
        for i, (letter, conf, x) in enumerate(line_chars):
            if i > 0:
                ...
                avg_char_width = np.mean(
                    [b[2] for b in boxes]) if boxes else 20
